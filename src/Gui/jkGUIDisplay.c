...
static jkGuiElement jkGuiDisplay_buttons2[25] =
{
	...
	{ ELEMENT_CHECKBOX,    0,            2, "GUI_3DACCEL",           0, {70,150,200,40 },    1, 0, NULL,                        0, jkGuiDisplay_something_d3d_check_related, 0, {0}, 0},
...
static jkGuiMenu jkGuiDisplay_menu = { jkGuiDisplay_buttons, 0, 0xFF, 0xE1, 0x0F, 0, 0, jkGui_stdBitmaps, jkGui_stdFonts, 0, 0, "thermloop01.wav", "thrmlpu2.wav", 0, 0, 0, 0, 0, 0 };

uint32_t jkGuiDisplay_deviceIdx = 0;
uint32_t jkGuiDisplay_deviceIdx3d = 0;
uint32_t jkGuiDisplay_hwModeListIdx = 0;
...
void jkGuiDisplay_PrecalcViewSizes()
{
	...
	stdVideoMode* vidMode = device->stdVideoMode;
...
void jkGuiDisplay_Startup()
{
	...
	Video_modeStruct.descIdx = stdDisplay_FindClosestMode(&renderSettings, &devices[closestDeviceIdx].stdVideoMode, devices[closestDeviceIdx].max_modes);

	if (devices[closestDeviceIdx].stdVideoMode[Video_modeStruct.descIdx].format.format.bpp != 8)
		Windows_GameErrorMsgbox("ERR_NEED_256_COLOR");

	Video_modeStruct.viewSizeIdx = 5;

	int loadedDisplayGUID = wuRegistry_GetBytes("displayDeviceGUID", (BYTE*)&Video_modeStruct.field_0C, 0x10);
	int loaded3DGUID = wuRegistry_GetBytes("3DDeviceGUID", (BYTE*)&Video_modeStruct.field_1C, 0x10);
	...
	if (loaded3DGUID == 0 || loadedDisplayGUID == 0)
	{
		...
		Video_modeStruct.field_0C = deviceList[Video_modeStruct.modeIdx].device.guid.Data1;
		Video_modeStruct.field_10 = deviceList[Video_modeStruct.modeIdx].device.guid.Data2;
		Video_modeStruct.field_14 = deviceList[Video_modeStruct.modeIdx].device.guid.Data3;
		Video_modeStruct.field_18 = deviceList[Video_modeStruct.modeIdx].device.guid.Data4;
	...
	else
	{
		...
		{
			currentDeviceGuidPtr = &deviceList->device.guid;
			do
			{
				int bGuidMatches = 0;
				...
				devices++;
				currentDeviceGuidPtr = (GUID*)((char*)currentDeviceGuidPtr + sizeof(stdVideoDevice));
				deviceIndex++;
			} while (deviceIndex < jkGuiDisplay_displayEnv->numDevices);
		}
	...
	#if 0
		{
			const char* magicGuidBytes = (const char*)&DAT_00522bf8;
			...
			{
				// Search device's hwModeList for matching 3D GUID
				int hwModeCount = devices->hwModeList;
				...
				{
					Video_modeStruct.field_0C = deviceList[deviceIndex].guid.a;
					Video_modeStruct.field_10 = deviceList[deviceIndex].guid.b;
					Video_modeStruct.field_14 = deviceList[deviceIndex].guid.c;
					Video_modeStruct.field_18 = deviceList[deviceIndex].guid.d;
					goto fallback_magic_defaults;
				}
			}
		}
	...
	while (deviceIndex < deviceCount && !foundMatch)
	{
		...
		if (dev->device.video_device[0].has3DAccel &&
			...
			dev->stdVideoMode != NULL)
		{
			stdVideoMode* modeList = dev->stdVideoMode;
	...
	if (!foundMatch)
	{
		// Secondary search in hwModeList arrays if no match above
		foundMatch = 0;
		deviceIndex = 0;
		hwModeListPtr = &deviceList->hwModeList;
	
		while (deviceIndex < deviceCount && !foundMatch)
		{
			modeInfoPtr = (int*)hwModeListPtr[1];
			int modeCount = *puVar10;
			int modeIdx = 0;
	
			while (modeIdx < modeCount && !foundMatch)
			{
				if (modeInfoPtr[0] != 0 && modeInfoPtr[1] != 0 && modeInfoPtr[2] != 0 &&
					((*(uint8_t*)(modeInfoPtr + 9) & 2) != 0) &&
					((*(uint8_t*)(modeInfoPtr + 10) & 0x10) != 0))
				{
					foundMatch = true;
					jkGuiDisplay_deviceIdx3d = deviceIndex;
					jkGuiDisplay_hwModeListIdx = modeIdx;
				}
				modeInfoPtr += 0x8b;
				modeIdx++;
			}
...
void jkGuiDisplay_Shutdown()
{
	...
	wuRegistry_SaveBytes("displayDeviceGUID", (BYTE*)&Video_modeStruct.field_0C, 16);
	wuRegistry_SaveBytes("3DDeviceGUID", (BYTE*)&Video_modeStruct.field_1C, 16);
...
void jkGuiDisplay_sub_4152E0(jkGuiElement* param_1, Darray* param_2, videoModeStruct* param_3)
{
	char* piVar1;
	int iVar2;
	stdVideoDeviceEntry* psVar3;
	char* pcVar4;
	intptr_t id;
	int local_88;
	int local_84;
	...
	jkGuiRend_DarrayFreeEntry(param_2);
	iVar2 = param_3->modeIdx;
	psVar3 = jkGuiDisplay_displayEnv->devices;
	jkGuiRend_AddStringEntry(param_2, "[SW] RenderDroid I", 0x45);
	if (param_3->Video_8605C8 == 0x45)
	{
		param_1->selectedTextEntry = 0;
	}
	id = 0;
	local_84 = 1;
	if (psVar3[iVar2].hwModeList != 0)
	{
		local_88 = 0;
		do
		{
			piVar1 = (psVar3[iVar2].field_2A4 + local_88);
			if (((*(byte*)(psVar3[iVar2].field_2A4 + 0x28 + local_88) & 0x10) != 0) &&
				((*(byte*)(piVar1 + 9) & 2) != 0))
			{
				pcVar4 = "HW";
				if (*piVar1 == 0)
				...
				_snprintf(local_80, 0x80, "[%s] %s", pcVar4, piVar1 + 0x12);
				jkGuiRend_AddStringEntry(param_2, local_80, id);
				if (param_3->Video_8605C8 == id)
					param_1->selectedTextEntry = local_84;

				local_84 = local_84 + 1;
			}
			id = id + 1;
			local_88 = local_88 + 0x22c;
		} while (id < psVar3[iVar2].hwModeList);
	}
	jkGuiRend_DarrayReallocStr(param_2, (wchar_t*)0x0, 0);
	jkGuiRend_SetClickableString(param_1, param_2);
	return;
}
...
void jkGuiDisplay_UpdateVideoModes(jkGuiElement* guiElement, Darray* stringArray, videoModeStruct* params)
{
	...
	stdVideoMode* videoMode = devices[deviceIndex].stdVideoMode;
...
int jkGuiDisplay_UpdateDeviceList()
{
	int result;
	stdVideoDevice* devices = jkGuiDisplay_displayEnv->devices;
	...
	if (numDevices > 0)
	{
		for (deviceId = 0; deviceId < numDevices; deviceId++, offset += 0x2A8)
		{
			stdVideoDevice* currentDevice = (stdVideoDevice*)((char*)devices + offset);

			const char* accelStr = "3D";
			if (currentDevice->video_device[0].has3DAccel == 0)
				accelStr = "Non-3D";

			const char* hwSwStr = "HW";
			if (currentDevice->video_device[0].hasGUID == 0)
				hwSwStr = "SW";

			_snprintf(entryStr, sizeof(entryStr), "[%s:%s] %s", hwSwStr, accelStr, currentDevice->driverName);
...
int jkGuiDisplay_Show()
{
	...
	jkGuiDisplay_buttons2[21].bIsVisible = Main_bDisplayConfig;
#ifdef USE_ORIGINAL
...
int jkGuiDisplay_something_d3d_check_related(jkGuiElement* param_1, jkGuiMenu* param_2, int param_3, int param_4, int param_5)
{
	...
	else
	{
		...
		Video_modeStruct.Video_8605C8 = jkGuiDisplay_hwModeListIdx;
	}
	Video_modeStruct.b3DAccel = (HKEY)!bVar3;
	Video_modeStruct.descIdx = 0;
	jkGuiDisplay_UpdateVideoModes(&jkGuiDisplay_buttons[6], &jkGuiDisplay_videoModeList, &Video_modeStruct);
	jkGuiRend_UpdateAndDrawClickable(jkGuiDisplay_buttons2 + 9, param_2, 1);
	psVar2 = jkGuiDisplay_displayEnv->devices;
	Video_modeStruct.field_0C = psVar2[Video_modeStruct.modeIdx].device.guid.Data1;
	Video_modeStruct.field_10 = psVar2[Video_modeStruct.modeIdx].device.guid.Data2;
	Video_modeStruct.field_14 = psVar2[Video_modeStruct.modeIdx].device.guid.Data3;
	Video_modeStruct.field_18 = psVar2[Video_modeStruct.modeIdx].device.guid.Data4;
	if (Video_modeStruct.Video_8605C8 == 0x45)
	{
		Video_modeStruct.field_1C = -0x78fae580;
		Video_modeStruct.field_20 = 0x11d113fc;
