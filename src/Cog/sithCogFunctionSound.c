    ...
    if ( pThing )
    {
        ...
        if (COG_SHOULD_SYNC(ctx))
        {
            if ( playingSound )
                ...
                refid_ = -1;
            sithDSSThing_SendPlaySound(pThing, &pThing->position, pSound, minDist_act, maxDist_act_, flagsTmp, refid_, INVALID_DPID, 255);
        }
    }
    else
    {
        ...
        if (COG_SHOULD_SYNC(ctx))
        {
            if ( playingSound )
                ...
                refid = -1;
            sithDSSThing_SendPlaySound(0, 0, pSound, volume, 0.0, flags, refid, INVALID_DPID, 255);
        }
    }
    if ( playingSound )
...
void sithCogFunctionSound_PlaySoundPos(sithCog *ctx)
{
    ...
    if (COG_SHOULD_SYNC(ctx))
    {
        ...
        sithDSSThing_SendPlaySound(0, &pos, pSound, minDist_act, maxDist_act, flagsTmp, refId, INVALID_DPID, 255);
    }
...
void sithCogFunctionSound_PlaySoundGlobal(sithCog *ctx)
{
    ...
    if ( playingSound )
    {
        if (COG_SHOULD_SYNC(ctx))
        {
            sithDSSThing_SendPlaySound(0, 0, pSound, volume, pan, flagsTmp, playingSound->refid, INVALID_DPID, 255);
        }
        sithCogExec_PushInt(ctx, playingSound->refid);
    }
    else
...
void sithCogFunctionSound_StopSound(sithCog *ctx)
{
    ...
    if ( playingSound && (playingSound->sound || playingSound->pSoundBuf) )
    {
        if (COG_SHOULD_SYNC(ctx))
        {
            sithDSSThing_SendStopSound(playingSound, fadeOut, INVALID_DPID, 255);
        }
        if ( fadeOut > 0.0 )
        {
