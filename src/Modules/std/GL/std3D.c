...
void std3D_SetTextureState(std3D_worldStage* pStage, std3D_DrawCallState* pState)
{
	...
	if (pTexture)
	{
		...
		{
			std3D_bindTexture(GL_TEXTURE_2D, tex_id, 0);
		...
		{
			std3D_bindTexture(GL_TEXTURE_2D, emiss_tex_id, 3);
		...
		{
			std3D_bindTexture(GL_TEXTURE_2D, displace_tex_id, 4);
	...
	else
	{
		...
		{
			std3D_bindTexture(GL_TEXTURE_2D, blank_tex, 3); // emissive
			std3D_lastTexBind[1] = blank_tex;
		}

		if (std3D_lastTexBind[2] != blank_tex)
		{
			std3D_bindTexture(GL_TEXTURE_2D, blank_tex, 4); // displace
			std3D_lastTexBind[2] = blank_tex;
		}

		if (std3D_lastTexBind[0] != blank_tex_white)
		{
			std3D_bindTexture(GL_TEXTURE_2D, blank_tex_white, 0);
...
inline uint32_t std3D_CheckDirtyState(std3D_DrawCall* pDrawCall, std3D_DrawCallState* pLastState, int lastShader, int texid)
{
	...
	dirtyBits |= (pLastState->stateBits.texMode != pDrawCall->state.stateBits.texMode) ? STD3D_TEXTURE : 0;
	dirtyBits |= (pLastState->stateBits.texGen != pDrawCall->state.stateBits.texGen) ? STD3D_TEXTURE : 0;
	dirtyBits |= (pLastState->stateBits.texFilter != pDrawCall->state.stateBits.texFilter) ? STD3D_TEXTURE : 0;
