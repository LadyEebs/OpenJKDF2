...
void sithExplosion_UpdateForce(sithThing *explosion)
{
    ...
    if ( range > 0.0 && (damage > 0.0 || force > 0.0) )
    {
        ...
        sithCollision_SearchRadiusForThings(explosion->sector, 0, &explosion->position, &rdroid_zeroVector3, 0.0, range, RAYCAST_400 | SITH_RAYCAST_COLLIDE_SPHERE_ONLY | SITH_RAYCAST_IGNORE_ADJOINS);
