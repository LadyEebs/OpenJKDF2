...
#define NETMSG_PUSHU64(x) {FIX_ALIGN(uint64_t); *(uint64_t*)craftingPacket = (uint64_t)(x); craftingPacket += sizeof(uint64_t);};
...
#define NETMSG_POPU64() (CHECK_ALIGN(uint64_t), _readingOutU64 = *(uint64_t*)_readingPacket, _readingPacket += sizeof(uint64_t), _readingOutU64)
...
void sithMulti_EndLevel(uint32_t, int a2);
