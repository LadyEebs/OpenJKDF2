...
int sithPuppet_FindHitLoc(sithThing* pReceiverThing, rdVector3* pPos)
{
	if (pReceiverThing->rdthing.model3 && pReceiverThing->animclass)
	{
		...
		for (int jointIdx = 0; jointIdx < ARRAY_SIZE(pReceiverThing->animclass->bodypart_to_joint); ++jointIdx)
		{
...
void sithPuppet_DebugDrawJointNames(sithThing* pThing)
{
	...
	for (int jointIdx = 0; jointIdx < ARRAY_SIZE(pThing->animclass->bodypart_to_joint); ++jointIdx)
	{
		int nodeIdx = pThing->animclass->bodypart_to_joint[jointIdx];
		if (nodeIdx < 0)
			continue;
